...
the project follows and is based on sebastian raschka's book, *build a large language model (from scratch)*.<br/>
...
3. **convert tokens into token IDs** -- map every token to an integer from the tokenizer's vocabulary<br/>
...
7. **load batches with a DataLoader** -- group several input-target pairs into batches<br/>
...
10. **combine the embeddings** -- add token embeddings and position embeddings together to form input embeddings<br/>
...
13. **create values** -- multiply the input embeddings by the value weight matrix<br/>
...
17. **calculate context vectors** -- use the attention weights to combine the value vectors<br/>
